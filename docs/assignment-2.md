...
- **Payload Teams** – interface with the satellite bus; require a standardized mechanical and electrical payload interface
...
- **Remote Sensors/Ground Users** (`rs`) – field devices and users that uplink relay messages. Obviously these are the `Users` and the `Remote Sensors` are the same as the stakeholders. They are part of the context as they interact directly with the satellite.
...
- **Environment** – decomposed into `Atmosphere`, `Earth`, and `Sun`, representing the physical environment. The elements highlighted as parts of the environment  were chosen for the following reasons: 
  ...
  - The satellite will be solar powered. Also the heating effects of prolonged exposure to solar radiation will affect how the satellite operates.
...
Requirements were derived from the nine stakeholder memos and organized into six categories. After the id there is a short summary of the requirement, some explanation on why they exist and with quotations from the memos and engineering comments that they were based on. The categories and key requirements are:
...
- REQ1.1 – Telemetry Service: the spacecraft shall transmit health information to ground operators. One can only maintain a satellite, or any system for that matter, if they have up-to-date information on the status(operational capabilities, component health, etc.) of it. In our case, this means frequent and punctual broadcasts of telemetry by the satellite. This was requested by the ground operators: "*We need regular status information so we know whether the spacecraft is alive.*"
- REQ1.2 – Store-and-Forward Message Handling: the system shall store messages until they can be downlinked. The satellite can't magically forward any message anywhere at any time, even if conditions are ideal, as the recipient might not be in range. This naturally means that for messages to be delivered,user and telemetry both, they need to be stored on-board for a time: "*Weather or operational issues may prevent successful downlink for up to 72 hours. Persistent onboard storage should therefore be sized to accommodate multiple days of accumulated traffic, including operational logs.*"
...
- REQ2 – Ground Contact Utilization: 2–4 passes per day, 5–10 minutes each is the expected amount of time that can be used for downlinking. The satellite is expected to be able to perform all the necessary broadcasting to the ground stations during this time."*Ground communication opportunities are limited; assuming approximately three ground station passes per day with 5–10 minutes of usable link time per pass is reasonable.*"
- REQ3 – Receiver Duty Cycle: 10–50% of each orbital period is the reasonable expected up-time for receiving of user messages, considering power constraints. "*assuming a 10–50 % receiver duty cycle is reasonable*" 
...
**Safety**
- REQ8 – End-of-Life Passivation: transmitters disabled and battery passivated at end of life. Responsible end of life behavior is expected. It is important to minimize orbital debris and to not have a noise generator stay in orbit."*End-of-life behavior includes transmitter shutdown and battery passivation.*"
...
**Reliability**
- REQ11 – Safe Mode: a protective mode that priorities survival and basic communications. Solar events and other disruptive happenings may occur. "*There should be a conservative configuration in which the spacecraft focuses solely on survival and communication with Earth.*"
...
- REQ13 – Fault Detection and Recovery: watchdog-based reset and fault recovery strategies. Radiation can, and therefore will, impact computation. This shouldn't end the mission. "*Radiation resets expected → watchdog/reset recovery typical.*"
...
The Use Case diagram captures the satellite's externally observable behaviors. 
...
**Exception path - Failed boot or health check on new image**
...
| Step  | Action | Results |
| ----- | ------------- | - |
|   1   |  Authorized operator prepares command with authentication fields | Command ready for uplink |
...
**Exception path - Link loss during uplink**
...
| Operator is authorized to send commands | Pre-condition | Approved |
...
| Step  | Action | Results |
| ----- | ------------- | - |
|   1   | Operator defines target parameters within license limits | Valid target set |
...
**Alternate – Parameter outside license or policy**
...
|   2   |  Rollback to last known good radio profile per policy | Safe RF behavior restored |
...
| Authorized command path is used (operator + authentication) | Pre-condition | Approved |
...
- **Remote Sensors** and **Users** → *Receive relay message, includes Store relay message* , *Forward relay data*: the core relay up- and downlink from field devices and users.
...
|   2   | Behavior matches documented energy and mode policy | No silent undefined state |
...
|   2   | Apply documented rule: e.g. drop oldest, drop lowest priority, or reject new | Behavior is deterministic |
...
|   2   | Enter safe handling per architecture (e.g. safe mode, relay paused) | Mission safety prioritized |
...
This diagram shows the realization relationships between port block types and their corresponding interfaces. Each port type (e.g., `Telemetry Port`, `Communication Port`, `Deployment Port`) is linked to its `«interface»` definition via a dashed realization arrow. This formalizes the type system used in the IBD connections and ensures that port compatibility can be checked for correctness and traceability.
...
- **Inhibit Release** – signal authorizing the transition from launch-safe to operational configuration
...
- **Wireless Interface** (`«interface»`) – models the radio-frequency deployment confirmation path; receives `Deployment Confirmation Signal` from the launch vehicle deployer. The `Wireless Connection` block type realizes this interface.
...
- **CanMessage** (`«signal»`) – the CAN bus message format, parameterized with `Header` and `Payload` arguments; used by the CAN Interface
...
## 20. System Architecture – Platform Configuration

![Platform Configuration](../renders/Model.Radio_Relay_Node_Satellite.System_Architecture.Platform_Configuration.Platform_Configuration.svg)

This Block Definition Diagram maps out the exact hardware instance configuration of the 3U CubeSat, defining the modular boundaries between the reusable Satellite Bus (Platform) infrastructure and the mission-specific Radio Network (RN) Node Payload.

The configuration documents the specific Commercial Off-The-Shelf (COTS) subsystem parts selected to achieve architectural closure within university-scale budgetary constraints:

- On Board Computer (OBC) – The central processing hub hosting the primary flight software stack (image_1a4d3f.png). It drives the main state machine, schedules data processing routines, handles incoming telecommands, and interfaces with hardware watchdog timers to manage automated subsystem resets.

- Electrical Power Subsystem (EPS) – The core power distribution unit regulating energy harvesting from the 3U solar panels. It monitors battery cell voltages and acts as a protection barrier against critical deep discharge states by power-gating non-essential loads.

- Communication Subsystem – A paired RF architecture consisting of a dedicated UHF/VHF Radio Transceiver for uplink reception/downlink transmission, connected directly to a deployable Antenna System via coaxial links.

- Sensors & Actuators – A localized instrumentation array containing dual redundant GPS receivers for orbital positioning and time synchronization, alongside rotation sensors and localized thermal probes to feed system health loops.

## 21. System Architecture – Platform Function 

### High Level

![Platform Function High Level](../renders/Model.Radio_Relay_Node_Satellite.System_Architecture.Platform_Functions.High_level_Functions.Platform_Specific_Functions.svg)

This diagram establishes the high-level Platform Independent Functions, decomposing the abstract operational capabilities the satellite must execute to maintain autonomy, survive the space environment, and fulfill its data relay mission:

- Command & Control – Decodes ground station commands, manages autonomous state transit logic, and maintains safe mode configurations during active system anomalies.

- Communication Management – Governs radio RF transmission properties (frequency, power steps, data rate tuning) and handles low-rate, high-robustness beacon broadcasts.

- Deployment Management – Handles post-launch countdown sequences, monitoring dual mechanical and software deployment inhibits to prevent premature radio transmission.

- Payload Data Management – Coordinates the core store-and-forward sequence, handling temporary data caching, storage limits, and persistent message validation queues.

- Power Management – Manages battery cycling profiles, balances solar generation margins against platform draws, and isolates power rails when low voltage states are detected.

- Telemetry & Health Monitoring – Regularly samples system metrics (voltages, current draws, operational logs) to package into recurring health beacons.

- Temperature Management – Active surveillance of internal thermal bounds (targeted between $-10^\circ\text{C}$ to $+50^\circ\text{C}$) to drive software-switched heaters or trigger payload emergency shutoffs.


### Low Level

![Platform Function Low Level](../renders/Model.Radio_Relay_Node_Satellite.System_Architecture.Platform_Functions.Low_level_functions.svg)

This view captures the Platform Specific Functions, illustrating how high-level logical behaviors inherit down to explicit low-level functional blocks that interact directly with physical hardware boundaries, ports, and IO lines:

- Port Mapping and Stereotypes – Specializes structural system functions by allocating them to dedicated system interfaces . The specialized low-level Payload Data Management and Communication Management blocks explicitly surface physical port interfaces, including the control port (com: Control Port) for command processing, alongside physical RF boundaries (rfout: Communication Port and rfrec: Communication Port) .

- Interface Realization – Maps abstract behavioral interactions down to exact system hardware lines (image_1a4d9f.png). High-level commands or storage transfers are serialized into physical data bus signals (such as CAN-bus transactions or serial packet payloads), creating a verifiable link between the abstract functional layer and the hardware communication buses.

![Platform Function](../renders/Model.Radio_Relay_Node_Satellite.System_Architecture.CubeSat_Platform_Function.CubeSat_Platform_Function.svg)


![Architecture](../renders/Model.Radio_Relay_Node_Satellite.System_Architecture.CubeSat_Architecture.CubeSat_Architecture.svg)
## 22. Reliability Analysis – Fault Tree Analysis
...
- *Lack of synchronization with Ground Station* — the satellite transmits but the ground station is not in a receiving configuration; an operational scheduling risk given the limited ~3 passes/day contact window (Assumption A8)
...
## Reliability Analysis - Fault Detection, Isolation and Recovery

To satisfy the safety and reliabilty requirements(REQ10-REQ15) and mitigate the failure paths identified in the Fault Tress(FT1-FT4), the architecture implements countermeasures.

### System-Level Fault Response (REQ10, REQ14)

The architecture is prepared to safeguard core functionality during undesirable events, such as the temperature being to hot or the battery power being low enough to trigger a safe mode. This mechanism implements (REQ14) by dropping out of full operation when conditions as the ones mentioned above are violated. By invoking *Payload_Power_OFF*, it strips power from the payload to conserve battery.

![StaeOperationModes](state_machines/StateOperationModes.png)

### Subsystem Fault Mitigation and Thermal Management (REQ11, REQ12)

#### Thermal SafeGuards (REQ11)

The HeaterOperation Modes state machine describes an autonomous thermal control loop designed to handle low-temperature anomalies:

- Low Thermal Threshold: if the current temperature is below 2 degrees, the system transitions to the Heating state
- High Thermal Reset: Once the local temperature has reached a temperature higher than 10 degrees it turns of the heating
- High Thermal Fault Mitigation: If the thermal environment spikes past 45 degrees the system reverts to the Safe mode 

![Heater Operation Modes](../renders/Model.Radio_Relay_Node_Satellite.Functional_Model.Functions.Temperature_Management.HeaterOperation_Modes.HeaterOperation_Modes.svg)

### Battery Discharge Protection (REQ12)

Power allocation relies on a reactive supply loop managed by the Electrical Power Subsystem (EPS):
- Transient Regulation: The system toggles dynamically between Battery Mode and Direct Solar Power based on the expression: $$\Delta P = \text{SolarPower\_Output} - \text{Power\_Input}$$
-Crtical Portection (FT2): If storage reserves go below 20%, the Telemetry & Health Monitoring (THM) logs the anomaly and the system triggers Safe Mode

![Power Supply Modes](../renders/Model.Radio_Relay_Node_Satellite.Functional_Model.Functions.Power_Management.Power_Supply_Modes.Power_Supply_Modes.svg)

### Onboard Computer Fault Recovery

To handle runtime errors such as cosmic radiation described in FT4 the THM utilizes a watchdog time mechanism.
- Detection: A failure to service the hardware watchdog tick withing the predefined timing window generates *WatchdogTimeout* event
- Isolation & Recovery: The system leaves *Normal Operation* and triggers a hardware reset and steps back into the initialization routine. This clears corrupted memory and forces to reinitialize.

![Telemetry&Health Monitoring](../renders/Model.Radio_Relay_Node_Satellite.Functional_Model.Functions.Telemetry___Health_monitoring.Telemetry___Health_monitoring.Telemetry___Health_monitoring.svg)


### Payload Data Manmagement and Messaging Integrity (REQ13)

The *Payload Data Management* activity diagram describes the message processing when faults or storage limits occur, possibly causing software freezes.
- Data Corruption Handling: When an incoming message corrupts mid-transit, the system evaluates and routes execution away from memory allocation, dropping the broken packet, logging the specific fault offset and update telemetry error frame.
- Out of Memory Handling (FT1): To prevent storage buffer crashes, the system checks whether the storage is full or not. Should it be full it throws a warning flag to the ground operators and deletes the oldest data to prevent crashes later on.

![Payload Data Management](../renders/Model.Radio_Relay_Node_Satellite.Functional_Model.Functions.Payload_Data_Management.Payload_Data_Management.svg)

### Mechanical Deployment Fault Management

Mechanical deployment mechanisms present risks during early-stage mission operations:
- Stuck Mechanism Exception (FT3); Upon time expiration, the *Deploy_Antenna_Cmd* command is given, advancing the antenna system into the *Deploying* state.
- timeout Mitigation:  If mechanical sensor does not trigger the *Deployment_Sensor_Triggered* confirmation before 60 seconds, execution drops to *Fault* state. This automatically halts power delivery to the actuators to avoid burnout or thermal warping, also alerting operators.

![Antenna System](../renders/Model.Radio_Relay_Node_Satellite.Functional_Model.Functions.Deployment_Management.Anntenna_System_StateMachine.Anntenna_System_StateMachine.svg)
